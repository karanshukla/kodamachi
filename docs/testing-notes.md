...
- **`--isolate`** — Bun's `mock.module` is process-global and **not restorable** (`clearAllMocks`/`mock.restore()` clear mock call history and restore spies but do not unmock modules). `--isolate` gives each test file a fresh module registry so a file's `mock.module` (e.g. `auth-service.test.ts` mocking `session-agent`) can't leak into other files that import the real module (e.g. `session-agent.test.ts`, which tests `initializeAgentFromSession` for real). Treat it as defence in depth, not a licence to write partial module mocks — see the `auth-service.test.ts` note above about spreading the real module into the mock. Bun 1.4.1 fixed `--isolate` leaking `mock()`, `spyOn()`, `mock.module()` and `Bun.plugin()` state across files, so the flag is doing more of what it claims than it was.
...
`bun test` accepts unknown flags **without erroring** (still true on 1.4, re-checked in #292). `--isolate` landed in Bun 1.3.14; on an older build the flag would be dropped silently. The real floor is now **Bun 1.4**, enforced by the runtime rather than a version check: the patched `@atproto-labs/fetch-node` imports `undici_v8` statically, which throws at module load on 1.3.x (see below). The former `src/tests/assert-bun-version.js` guard was removed in #288 along with the shim, since the Bun-only suite has no second runtime to fall back to. `@types/bun` pins `1.4.0` in `server/package.json` and the root, which is the newest DefinitelyTyped publish; the types lag the runtime by a patch and there is no `1.4.1` to move to. CI installs `bun-version: latest` so it keeps tracking Bun, and the Dockerfiles pin the `oven/bun:1.4-alpine` minor track, which now resolves to 1.4.1.
...
3. **`coverageThreshold` is enforced per file, not on the total, so none is set.** This was previously recorded here as a Bun bug ("any value exits 1 below 100%, whatever the threshold says"). That was a misreading. The threshold is applied to every file individually, so a run whose aggregate clears the bar still exits 1 if any single file is under it, which is what `0.97` against an aggregate of 99.59% was. Behaviour is identical on 1.3.14 and 1.4.1; nothing about it changed. Re-verified on 1.4.1 with a two-file fixture (`hi.ts` 100% funcs, `lo.ts` 50%, reported total 75%): threshold `0.4` exits 0, `0.6` exits 1. Two further traps if this is ever turned on: in the per-metric object form the keys are plural (`lines`, `functions`) and the singular `line`/`function` are silently ignored, and **an omitted metric defaults to 1.0**, so `{ functions = 0.4 }` alone still fails a file at 60% lines. CLI flags like `--coverage-threshold=` are genuinely not honored; the setting only reads from `bunfig.toml`. The bar stays on Coveralls regardless, so turning this on would be belt-and-braces, not a fix.

### The server suite went ungated for one release, and how

Setting a `coverageThreshold` meant `bun run test:coverage` always exited 1 (some individual file was always under the bar, per limitation 3 above), so the CI step carried `continue-on-error: true` plus a `Fail if tests failed` step keyed on `steps.server-tests.outcome`. Neither worked. The step body piped through `tee` without `set -o pipefail`, so the step's exit status was always `tee`'s `0` — the outcome was never `failure`, the guard never fired, and the `continue-on-error` was moot. Any failing server test reported green.
...
Fixed by removing the threshold, which lets the exit code track test results, then dropping `continue-on-error` and the dead guard step and adding `set -o pipefail`. The Coveralls gate is unchanged and still bars a coverage drop. The load-bearing fix was `pipefail` plus dropping `continue-on-error`: with those in place a threshold could be reintroduced without hiding test failures, but there is no reason to, since Coveralls already gates coverage.
...
`bun test --coverage` (what the server uses) only measures code run by Bun's own test runner. The client suite is Vitest with happy-dom, `@testing-library/react`, and MSW, so using it would mean porting 545 tests across 36 files off Vitest. Not worth it to avoid an npm dependency, and it would be a step down on metrics: Bun's lcov has no branch data at all, and its `coverageThreshold` is enforced per file rather than on the total, which is why the server sets none (see "Coverage under Bun" above).
...
Only the last combination breaks, so this is Vitest's module runner over the prebundled dependency on Bun, not Bun's ESM loader and not Vite's transform. The namespace import is the form zod's own docs use, costs nothing, and sidesteps it. The regression guard is CI running the suite on Bun: reverting to `import { z }` fails it immediately. Re-checked on Bun 1.4.1, still broken: reverting `Login.tsx` to `import { z }` fails 28 tests with `TypeError: undefined is not an object (evaluating 'z.string')`. The probe script deliberately does not assert this, since a bare `import("zod")` inside the probe passes on both runtimes and would give false assurance.
...
Our real specs do not build. Five of the seven fail, and they are exactly the five that use an inline `type` import (`import { test, expect, type Page } from "@playwright/test"`); the two using a plain value import (`happy-path.spec.ts`, `web/navigation.spec.ts`) both pass. Re-checked on Bun 1.4.1, unchanged:
...
AggregateError: 2 errors building "e2e/web/settings.spec.ts"
