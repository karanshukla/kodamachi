## Backend Server App for kodamachi
...
Session management is kept intentionally thin on the server side for speed and simiplicitly. The app is constantly refreshing its session with the actual Bluesky OAuth service while the kodamachi user is logged in, so if there are any issues, the app will log out the end user and trigger a reauthentication. In a sense, Bluesky itself is an authorization proxy. If your session is invalidated in Bluesky, its also invalidated in kodamachi.
