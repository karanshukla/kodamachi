...
This stack is used for **local integration testing and smoke tests** (and optionally self-hosting). It mirrors production closely: Railway builds every service from these same Dockerfiles (the builder and Dockerfile path for every service are pinned in `.railway/railway.ts`; see `CLAUDE.md` → "Deployment (Railway)").
