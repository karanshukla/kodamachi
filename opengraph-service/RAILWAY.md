...
| `PUBLIC_URL`             | `https://kodamachi.app` | Public site origin. Used to build absolute `og:image` URLs (the cached PNG is served at `$PUBLIC_URL/og-cache/<did>.png`). Set this to your production domain.                                                                                                                                                     |
...
2. **Repoint Caddy** (the _existing_ Caddy service, not this one): change its
...
3. **Confirm `/api/*` is unaffected**: Caddy's `handle_path /api/*` routes to
   the server _before_ the `/*` reverse-proxy, so API traffic never enters
